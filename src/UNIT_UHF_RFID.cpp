...
void JRD_4035::cleanBuffer() {
    for(int i = 0; i<200; i++) {
        buffer[i] = 0;
    }
}

void JRD_4035::cleanCardsBuffer() {
    for(int i =0 ; i<200; i++){
        cards[i] = {
            rssi: 0,
            pc: {0},
...
bool JRD_4035::waitMsg(unsigned long time) {
...
void JRD_4035::sendCMD(uint8_t *data, size_t size) {
    _serial->write(data, size);
}


bool JRD_4035::filterCardInfo(String epc){
    for(int i=0; i<200; i++){
...
bool JRD_4035::saveCardInfo(CARD *card){
...
uint8_t JRD_4035::pollingMultiple(uint16_t polling_count) {
    cleanCardsBuffer();
...
bool JRD_4035::select(uint8_t *epc) {
